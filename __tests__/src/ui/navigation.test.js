...
describe('navigation', () => {
  ...
  it('supports keyboard activation without interfering with native buttons', () => {
    ...
    button.addEventListener('click', nativeSpy);
  ...
  it('delegates action, input and keyboard events through data contracts', () => {
    const cases = [
      ['toggle-verb', { index: '2' }, 'toggleVerbCard', 2],
      ...
      ['select-option', { index: '1' }, 'selectOption', 1],
      ['select-test-option', { index: '2' }, 'selectTestOption', 2],
      ['show-module', { index: '3' }, 'showModule', 3],
    ...
    for (const [action, data, mockName, arg] of cases) {
      ...
      if (arg !== undefined) expect(mocks[mockName]).toHaveBeenCalledWith(expect.anything(), arg);
    }
    ...
    expect(mocks.validateExercise).toHaveBeenCalled();
    expect(mocks.validateTestAnswer).toHaveBeenCalled();
  });
});
